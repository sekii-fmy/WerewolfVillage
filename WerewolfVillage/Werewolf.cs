...
namespace WerewolfVillage
{
    class Werewolf
    {
        ...
        GameData gameData;
        RecoverGeneration recoverGeneration = new RecoverGeneration();
        ...
        static double exchangeGeneration = 3;      //交流までの世代数
        ...
        public void continueGeneration(int village, int generation)
        {
            int exchange = 0;
            overGene(village, generation);
            printSurvivalAgent(generation);
            ...
            while (generation < generationNumbers)
            {
                ...
                exchange++;
                selection(generation, exchange);
                if ((generation % exchangeGeneration == 0) && generation < generationNumbers)
                {
                    exchangeGene();
                    exchange = 0;
                }
        ...
        public void overGene(int village, int generation)
        {
            for (int i = 0; i < villageNumbers; i++)
            {
                ...
                {
                    Agent agent = new Agent(new GeneOfParameter(), (i+1)*(k+1));
                    villager.Add(agent);
                }
                villager = recoverGeneration.ReadGeneFile(i, generation, villager);
        ...
        public GeneOfParameter cross(Agent villageAgent, Agent wolfAgent, Agent common )
        {
            ...
            gene.parameter[GeneOfParameter.length_gene] = wolfAgent.parameter.parameter[GeneOfParameter.length_gene];
            ...
            gene.parameter[88] = villageAgent.parameter.parameter[88];
            gene.parameter[89] = villageAgent.parameter.parameter[89];
            gene.parameter[GeneOfParameter.length_gene + 1] = villageAgent.parameter.parameter[GeneOfParameter.length_gene + 1];

            gene.parameter[8] = common.parameter.parameter[8];
            ...
            gene.parameter[GeneOfParameter.length_gene + 2] = common.parameter.parameter[GeneOfParameter.length_gene + 2];

            if (villageAgent.parameter.parameter[GeneOfParameter.length_gene + 3] == 1 || wolfAgent.parameter.parameter[GeneOfParameter.length_gene + 3] == 1 || common.parameter.parameter[GeneOfParameter.length_gene + 3] == 1 )
            {
                gene.parameter[GeneOfParameter.length_gene + 3] = 1;
            }
